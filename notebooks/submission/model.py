...
class Model:
    ...
    def fit_epoch(self, dataset, n_batch=1, shuffle=False):
        ...
        for batch_idx, (X_batch, y_batch) in enumerate(dataset):
            ...
            y_batched = torch.zeros((y_batch.shape[0], 2, y_batch.shape[2], y_batch.shape[3]))
            y_batched[:, 0, :, :] = 1 - y_batch[:, 0, :, :]
            y_batched[:, 1, :, :] = y_batch[:, 0, :, :]
            y_batched = y_batched.to(device=self.device)

            training_loss = self.loss(y_out[:, 1, :, :], y_batched[:, 1, :, :])
    ...
    def predict_dataset(self, dataset, export_path):
        ...
        predictions = []
        for batch_idx, (X_batch, *rest) in enumerate(dataset):
            ...
            y_out = (y_out * 65535).astype(np.uint16)[0, 1, :, :]

            io.imsave(os.path.join(export_path, image_filename), y_out, check_contrast=False)

        # Save as numpy arr
        predictions = np.array(predictions)
        predictions = predictions.reshape(predictions.shape[0], predictions.shape[2], predictions.shape[3], predictions.shape[4])
        return predictions

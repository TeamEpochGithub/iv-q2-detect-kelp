...
def hash_scaler(cfg: DictConfig) -> str | None:
    ...
    has_steps = has_pretrain and len(pretrain.steps) > 0
    has_scaler = False if not (has_pretrain and has_steps) else next(iter(pretrain.steps)).get("scaler") is not None
    if has_scaler:
        scaler_hash = hash(
